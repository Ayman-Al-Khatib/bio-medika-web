...
class Validation {
  static String? isPhoneNumberValid(String? phoneNumber) {
    if (phoneNumber == null || phoneNumber == '') return AppTextsEnglish.This_field_is_required.tr;
    if (!phoneNumber.startsWith("09")) return AppTextsEnglish.Phone_must_start_with_09.tr;
    if (phoneNumber.length < 10) return AppTextsEnglish.Phone_must_be_10_characters_long.tr;
    return null;
  }

  static String? checkPasswordStrength(String? password, {bool isLogin = false}) {
    if (password == null || password == '') return AppTextsEnglish.This_field_is_required.tr;
    if (password.length < 8) return AppTextsEnglish.Password_should_be_at_least_8_characters.tr;
  ...
  static String? checkPasswordMatch(String? password, String? confirmPassword) {
    if (confirmPassword == null || confirmPassword == '') return AppTextsEnglish.This_field_is_required.tr;
  ...
  static String? validateEmail(String? email) {
    ...
    if (email == null || email == '') return AppTextsEnglish.This_field_is_required.tr;
  ...
  static String? length(String? text, {int min = 3, int max = 500}) {
  ...
  static String? dateAfterTodayValidator(String? value) {
    if (value == null || value.isEmpty) {
      return AppTextsArabic.This_field_is_required.tr;
    }
    DateTime? inputDate = DateTime.tryParse(value);
    if (inputDate == null) {
      return AppTextsArabic.Invalid_date_format.tr;
    }
    if (inputDate.isBefore(DateTime.now())) {
      return AppTextsArabic.Date_must_be_after_today.tr;
    }
    return null;
  }
}
